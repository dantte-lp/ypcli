...
profiles:
  work:
    ...
    vault:                     # источник Vault/OpenBao для `send --vault-path`
...
## Источник Vault / OpenBao

Профиль (или глобальный `defaults`) может содержать блок `vault` с настройками
подключения для чтения payload через `send --vault-path`:

| Поле | Значение |
|---|---|
| `addr` | адрес Vault/OpenBao |
| `mount` | mount KV v2 (по умолчанию `secret`) |
| `namespace` | namespace Vault/OpenBao |
| `token_command` | команда, выводящая токен Vault/OpenBao (не хранится) |

Задаётся через `ypcli config add <name> --vault-addr … --vault-mount … --vault-namespace … --vault-token-command …`
(или `ypcli config defaults --vault-…`). После этого в командной строке нужно
указать только расположение конкретного секрета:

```bash
ypcli send --profile work --vault-path db --vault-field password
```

Приоритет для каждой vault-настройки: флаг > env (`VAULT_*` / `BAO_*`) > блок
`vault` профиля. Токен vault дополнительно берётся из `token_command` блока.
См. [Использование](03-usage.md#из-менеджера-секретов-vault--openbao).
